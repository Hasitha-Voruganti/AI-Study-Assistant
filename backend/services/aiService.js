...
const answerQuestion = async (question, relevantChunks, mode = 'default') => {
  ...
  if (mode === 'simple') modeInstruction = 'Explain in very simple language for a beginner. Use short sentences. Avoid jargon.';
  if (mode === 'example') modeInstruction = 'Give a real-world example to illustrate the answer.';
...
Output ONLY the JSON array.`;
    ...
    try {
      ...
      const valid    = parsed
        ...
        .map(c => ({
          ...
          back:  String(c.back  || c.answer   || c.definition || 'Definition').trim()
        }));
      allCards.push(...valid);
    } catch (err) {
  ...
  if (allCards.length === 0) throw new Error('Failed to generate any flashcards. Please try again.');
  return allCards.slice(0, numCards);
};

const detectWeakTopics = async (quizHistory) => {
  const summary = quizHistory.map(q => `Topic: ${q.topic}, Score: ${q.percentage}%`).join('\n');
  const system  = 'You are a learning analytics assistant. Be concise and actionable.';
  const prompt  = `Quiz history:\n${summary}\n\nList weak topics (below 60%) with one specific study tip each.`;
  return callAI(system, prompt, 400);
};

module.exports = { callAI, answerQuestion, generateQuiz, generateFlashcards, detectWeakTopics };
